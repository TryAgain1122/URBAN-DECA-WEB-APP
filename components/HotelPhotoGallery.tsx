  ...
  return (
    ...
    <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex space-x-2">
        {images.map((_, index) => (
            <div
                key={index}
                className={`w-3 h-3 bg-white rounded-full ${index === currentIndex ? 'opacity-100' : 'opacity-50'} cursor-pointer`}
    ...
    {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75">
                    <div className="relative">
                        <img src={images[currentIndex]?.url} alt={`Slide ${currentIndex}`} className="w-full max-w-3xl max-h-screen object-contain rounded-md shadow-md" />
                        <button
                            onClick={closeModal}
